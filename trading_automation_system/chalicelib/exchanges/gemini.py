...
class GeminiOrderError(OrderError):
    """Raised when order creation fails."""
    pass

class GeminiClient:
    """
    Client for interacting with Gemini cryptocurrency exchange.

    The client handles all interactions with the Gemini exchange including trades,
    ...
    def __init__(self, quote_currency: str):
        ...
        self._client: Optional[ccxt.gemini] = None

    @property
    def client(self) -> ccxt.gemini:
        """Get the connected Gemini client."""
        if self._client is None:
            raise GeminiConnectionError("Client not connected. Call connect() first.")
        return self._client
    ...
    def connect(
        ...
        """
        Establish connection to Gemini exchange.
        ...
        Raises:
            GeminiConnectionError: If connection fails after max retries
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise GeminiConnectionError(f"Unexpected error: {str(e)}") from e
    ...
    def create_limit_order(
        ...
        """
        Place a limit order on Gemini exchange.
        ...
        Raises:
            GeminiOrderError: If order creation fails
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise GeminiOrderError(
                        f"Failed to place order after {max_retries} attempts: {str(e)}"
                    ) from e
                time.sleep(self.RETRY_DELAY)
                
            except ExchangeError as e:
                raise GeminiOrderError(f"Exchange rejected order: {str(e)}") from e
                
            except Exception as e:
                raise GeminiOrderError(f"Unexpected error placing order: {str(e)}") from e
    ...
    def get_order_status(self, order_id: str, max_retries: int = 3) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise GeminiConnectionError(f"Failed to fetch status for order {order_id}: {str(e)}") from e
    ...
    def get_total_base_asset(self, base_asset: str, max_retries: int = 3) -> Optional[Decimal]:
        ...
        Raises:
            GeminiConnectionError: If balance fetch fails after max retries
        """
        for attempt in range(max_retries):
            try:
                ...
                raise GeminiConnectionError(f"Failed to fetch balance: {str(e)}") from e
    ...
    def get_last_price(self, symbol: str, max_retries: int = 3) -> Optional[Decimal]:
        ...
        Raises:
            ...
            GeminiConnectionError: If price fetch fails after max retries
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise GeminiConnectionError(f"Failed to fetch price: {str(e)}") from e
    ...
    def get_account_allocation(self, max_retries: int = 3) -> Dict[str, Decimal]:
        ...
        Raises:
            GeminiConnectionError: If fetching data fails after max retries
        """
        for attempt in range(max_retries):
            try:
                ...
                raise GeminiConnectionError(f"Failed to fetch allocations: {str(e)}") from e
    ...
    def get_total_usd(self) -> Decimal:
        ...
        Raises:
            GeminiConnectionError: If fetching allocations fails
        """
    ...
    def get_most_recent_trade(self, symbol: str, max_retries: int=3) -> List[Dict]:
        ...
        Raises:
            GeminiConnectionError: If fetching trades fails after max retries
        """
        for attempt in range(max_retries):
            try:
                ...
                raise GeminiConnectionError(f"Failed to fetch trades: {str(e)}") from e
