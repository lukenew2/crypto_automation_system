...
class BinanceOrderError(OrderError):
    """Raised when order creation fails."""
    pass

class BinanceClient:
    """
    Client for interacting with Binance cryptocurrency exchange.

    The client handles all interactions with the Binance exchange including trades,
    ...
    def client(self) -> ccxt.binance:
        """Get the connected Binance client."""
    ...
    def connect(
        ...
        Raises:
            BinanceConnectionError: If connection fails after max retries
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise BinanceConnectionError(f"Unexpected error: {str(e)}") from e
    ...
    def create_limit_order(
        ...
        """
        Place a limit order on Binance exchange.
        ...
        Raises:
            BinanceOrderError: If order creation fails
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise BinanceOrderError(f"Exchange rejected order: {str(e)}") from e
                
            except Exception as e:
                raise BinanceOrderError(f"Unexpected error placing order: {str(e)}") from e
    ...
    def get_order_status(self, order_id: str, max_retries: int = 3) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise BinanceConnectionError(f"Failed to fetch status for order {order_id}: {str(e)}") from e
    ...
    def get_total_base_asset(self, base_asset: str, max_retries: int = 3) -> Optional[Decimal]:
        ...
        Raises:
            BinanceConnectionError: If balance fetch fails after max retries
        """
        for attempt in range(max_retries):
            try:
                ...
                raise BinanceConnectionError(f"Failed to fetch balance: {str(e)}") from e
    ...
    def get_last_price(self, symbol: str, max_retries: int = 3) -> Optional[Decimal]:
        ...
        Raises:
            ...
            BinanceConnectionError: If price fetch fails after max retries
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise BinanceConnectionError(f"Failed to fetch price: {str(e)}") from e
    ...
    def get_account_allocation(self, max_retries: int = 3) -> Dict[str, Decimal]:
        ...
        Raises:
            BinanceConnectionError: If fetching data fails after max retries
        """
        for attempt in range(max_retries):
            try:
                ...
                raise BinanceConnectionError(f"Failed to fetch allocations: {str(e)}") from e
    ...
    def get_total_usd(self) -> Decimal:
        ...
        Raises:
            BinanceConnectionError: If fetching allocations fails
        """
    ...
    def get_most_recent_trade(self, symbol: str, max_retries: int=3) -> List[Dict]:
        ...
        Raises:
            BinanceConnectionError: If fetching trades fails after max retries
        """
        for attempt in range(max_retries):
            try:
                ...
                raise BinanceConnectionError(f"Failed to fetch trades: {str(e)}") from e
